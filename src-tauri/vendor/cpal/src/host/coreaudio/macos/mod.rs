...
where
    E: FnMut(StreamError) + Send + 'static,
{
    // DM-Voice patch: hold only a Weak ref to the stream inside the listener
    // closure. The upstream code cloned the Stream (a strong Arc) and stored it
    // in StreamInner._disconnect_listener — which itself lives inside that same
    // Arc — creating a reference cycle. The strong count then never reaches zero,
    // so StreamInner (and the AudioUnit it owns) is never dropped/disposed. On
    // macOS that leaves the input device open after the Stream is dropped, keeping
    // the orange microphone indicator lit. This only affected non-default devices,
    // since add_disconnect_listener is called only for `!is_default`.
    let weak_inner = Arc::downgrade(&stream.inner);
